...
| [`use-case-discovery-board/`](use-case-discovery-board/) | A workshop canvas for the session that comes *before* prioritization: draw the client's as-is workflow as connected blocks, mark friction on the steps that carry it, attach the systems and data each step touches, then hang candidate AI use cases off the pain they address. Reads the board back as a discovery document and exports the drawing as SVG. Ships three worked examples — insurance claims, a furniture supply chain and a chilled-food supply chain — so the method can be shown on a process the room recognises. For a room brainstorming together, an optional [`workshop-relay.py`](use-case-discovery-board/workshop-relay.py) runs on the facilitator's own laptop and lets several people edit one board at once. Where the network is against you — client guest wifi usually is — groups can instead map in parallel and merge their exported boards, which nothing can block. See its [README](use-case-discovery-board/README.md). |
| [`use-case-prioritization/`](use-case-prioritization/) | SOP + a repeatable use-case matrix scored on ROI, feasibility and impact, ranking candidates so the engagement starts on the one most likely to succeed. Candidates mapped on the discovery board import straight in — value becomes ROI, effort inverts into feasibility, and impact is left for the architect to score. |
...
All six tools follow the toolkit-wide pattern: a checkable SOP checklist, an
autosaved client intake, a live document preview, HTML/Markdown export, a
"🧪 Load sample" button, an "Agent drafting" prompt generator, and an
